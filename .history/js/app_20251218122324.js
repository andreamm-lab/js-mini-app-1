...
const startBtn = startLabels.find((l) => l.textContent.trim().toLowerCase() === "iniciar");
...
function getTotalQuestions() {
  ...
  if (mode.includes("20")) return Math.min(20, QUESTIONS.length);
  return QUESTIONS.length;
}

function applyDifficulty() {
  const diff = difficultySelect.value.toLowerCase();
  if (diff.includes("fácil")) timePerQuestion = 12;
  else if (diff.includes("media")) timePerQuestion = 10;
